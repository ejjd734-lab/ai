...
def test_additional_security_headers_present(client):
    resp = client.get("/api/health")
    # HSTS enforces HTTPS
    ...
    assert "form-action" in resp.headers.get("Content-Security-Policy", "")


def test_oversized_request_body_rejected(client):
    # Send a content-length header that exceeds 1 MB limit.
    resp = client.post(
        "/api/calculate",
        content=b"x" * 10,
